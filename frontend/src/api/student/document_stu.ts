...
export const uploadDocument = async (file: File, title: string): Promise<UploadResponse> => {
    try {
        ...
        const response = await api.post<UploadResponse>('/student/document_vectorization/upload', formData, {
            headers: {
...
export const getDocumentList = async (): Promise<DocumentInfo[]> => {
    try {
        const response = await api.get<DocumentListResponse>('/student/document_vectorization/list')
...
export const searchDocuments = async (keyword: string, limit: number = 20): Promise<DocumentInfo[]> => {
    try {
        const response = await api.get<SearchResponse>('/student/document_vectorization/search', {
...
export const deleteDocument = async (documentId: string): Promise<string> => {
    try {
        const response = await api.delete<DeleteResponse>(`/student/document_vectorization/delete/${documentId}`)
