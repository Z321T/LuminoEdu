...
const downloadErrorReport = () => {
  if (!importResult.value || !importResult.value.failed_records || importResult.value.failed_records.length === 0) {
    errorMsg.value = '没有错误记录可下载'
    return
  }

  try {
    const errorData = importResult.value.failed_records.map((record: any, index: number) => ({
      序号: index + 1,
    ...
    link.click()
    window.URL.revokeObjectURL(url)
    showQuickTipMessage('📋 错误报告已下载')
  } catch (error) {
    errorMsg.value = '下载错误报告失败'
  }
}
